...
# Configuration
# Bechdel runs on dockge container (CT 111) on prxbox1
...
if ssh ${PROXMOX_USER}@${PROXMOX_HOST} "pct exec ${CONTAINER_ID} -- bash -c 'ping -c 1 -W 2 8.8.8.8 > /dev/null 2>&1'"; then
    ...
    ssh ${PROXMOX_USER}@${PROXMOX_HOST} "pct exec ${CONTAINER_ID} -- bash -c 'ip link set eth0 up 2>/dev/null || true; ip addr show eth0 | grep -q \"${CONTAINER_IP}\" || ip addr add ${CONTAINER_IP}/24 dev eth0; ip route show | grep -q default || ip route add default via 192.168.0.1'" || {
...
if [ "$HEALTHY" = true ]; then
    ...
    # Get HTTP status
    HTTP_STATUS=$(curl -sI http://${CONTAINER_IP}:8080 | head -1)
...
echo "  Local:  http://${CONTAINER_IP}:8080"
...
echo "  View logs:    ssh root@${PROXMOX_HOST} 'pct exec ${CONTAINER_ID} -- bash -c \"cd ${APP_PATH} && docker compose logs -f app\"'"
echo "  Restart app:  ssh root@${PROXMOX_HOST} 'pct exec ${CONTAINER_ID} -- bash -c \"cd ${APP_PATH} && docker compose restart app\"'"
echo "  Check status: curl http://${CONTAINER_IP}:8080/health"
echo ""
