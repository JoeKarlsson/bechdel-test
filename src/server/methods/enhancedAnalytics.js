	...
	async analyzeScript(scriptPath, characters, processId = null, bechdelData = null) {
		try {
			...
			if (processId) {
				...
				cleanupManager.setProcessMessage(processId, 'Running advanced AI analysis (including Bechdel validation)...');
			}

			// Run Claude AI analysis (includes Bechdel validation if bechdelData provided)
			const aiAnalysis = await this.claudeAnalyzer.analyzeScript(cleanedScriptData, characters, bechdelData);
