...
const BechdelCharts = ({ bechdelResults }) => {
	...
	// Dialogue data with percentages
	...
	const femaleDialoguePercent = totalLines > 0
		...
		: 0;
	const maleDialoguePercent = totalLines > 0
		? Math.round((totalLinesMaleDialogue / totalLines) * 100)
		: 0;

	const dialogueData = [
		{
			name: 'Female',
			lines: totalLinesFemaleDialogue,
			percentage: femaleDialoguePercent,
			color: femaleColor
		},
		{
			name: 'Male',
			lines: totalLinesMaleDialogue,
			percentage: maleDialoguePercent,
			color: maleColor
		},
	];

	// Calculate 50% reference line value for dialogue chart
	const equalRepresentationValue = totalLines / 2;
			...
			>
				...
				{hasCharacterData && (
					...
					>
							...
							<PieChart>
								...
								<Tooltip
									content={({ active, payload }) => {
										if (active && payload && payload.length) {
											const total = numOfFemalesChars + numOfMaleChars;
											...
											return (
												<div className="custom-tooltip">
													...
													<p className="value">{payload[0].value} characters ({percent}%)</p>
												</div>
											);
										}
										return null;
						...
						<p className="chart-annotation">
							👥 <strong>Representation Matters:</strong> The ratio of male to female characters reflects broader patterns in media representation and influences how audiences perceive gender roles.
						</p>
					</div>
				)}
					...
					>
						...
						<ResponsiveContainer width="100%" height={250}>
							<BarChart data={dialogueData}>
										...
										if (active && payload && payload.length) {
											return (
												<div className="custom-tooltip">
													...
													<p className="value">{payload[0].value} lines ({payload[0].payload.percentage}%)</p>
												</div>
											);
										}
										return null;
								...
								/>
								{/* Reference line at 50% (equal representation) */}
								{totalLines > 0 && (
									<ReferenceLine
										y={equalRepresentationValue}
										stroke={isDarkMode ? '#fbbf24' : '#f59e0b'}
										strokeDasharray="5 5"
										strokeWidth={2}
									>
										<Label
											value="50% Equal Representation"
						...
						<p className="chart-annotation">
							📊 <strong>Feminist Analysis:</strong> Research shows women receive approximately 30-35% of dialogue in Hollywood films. Equal representation (50%) remains rare, reflecting systemic inequality in narrative cinema.
						</p>
					</div>
				)}
				...
				{hasSceneData && (
					...
					>
						...
						<ResponsiveContainer width="100%" height={250}>
							<BarChart data={sceneData}>
								...
								<Tooltip
									content={({ active, payload }) => {
										if (active && payload && payload.length) {
											const total = numScenesPass + numScenesDontPass;
											const percent = total > 0
												? Math.round((payload[0].value / total) * 100)
												: 0;
											return (
												<div className="custom-tooltip">
													<p className="label">{payload[0].payload.name}</p>
													<p className="value">{payload[0].value} scenes ({percent}%)</p>
												</div>
											);
										}
										return null;
						...
						<p className="chart-annotation">
							🎬 <strong>The Test in Action:</strong> A scene passes when at least two named women have a conversation about something other than a man. Even one passing scene represents progress in female representation.
						</p>
					</div>
				)}
				...
				{hasCharactersWithDialogueData && (
					...
					>
						...
						<ResponsiveContainer width="100%" height={250}>
							<BarChart data={charactersWithDialogueData}>
								...
								<Tooltip
									content={({ active, payload }) => {
										if (active && payload && payload.length) {
											const total = numOfFemalesCharsWithDialogue + numOfMaleCharsWithDialogue;
											const percent = total > 0
												? Math.round((payload[0].value / total) * 100)
												: 0;
											return (
						...
						<p className="chart-annotation">
							💬 <strong>Voice & Agency:</strong> Having dialogue is the first step toward female agency in narrative cinema. Characters who speak can influence the plot and express independent perspectives.
						</p>
					</div>
				)}
